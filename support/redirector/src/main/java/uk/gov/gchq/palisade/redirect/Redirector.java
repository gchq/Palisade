...
/**
 * The base interface for a redirector. A redirector knows how to re-direct requests based on some specific algorithm and/or
 * metadata. For example, a round-robin redirector may simply send successive requests to the next host on a circular list,
 * whereas a redirector instance may redirect based on load on the respective hosts, while another may take data locality
...
public interface Redirector<T> {
     ...
     * any Palisade service, then this method carries both a reference to the original method called as well all the
     ...
     */
    RedirectionResult<T> determineRedirection(final Method method, final Object... args);
}
