 ...
 * service will require based on a <code>registerDataRequest</code> made to the palisade
...
public interface CacheService extends Service {
    /**
     * Adds a record to the cache according to the contents of the {@link AddCacheDataRequest}
     * parameter. The AddCacheDataRequest will contain a identifier that is unique
     * per client request for data and therefore it is a good candidate for a
     * lookup key.
     *
     * @param request {@link AddCacheDataRequest} containing a unique requestId and
     ...
     * contained in the {@link GetCacheDataRequest} parameter.
    ...
    default CompletableFuture<?> process(final Request request) {
        if (request instanceof AddCacheDataRequest) {
            add((AddCacheDataRequest) request);
            return null;
        }
