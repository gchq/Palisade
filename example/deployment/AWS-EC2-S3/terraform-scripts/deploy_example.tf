...
# NOTE: the current directory when running this will be example/deployment/AWS-EC2-S3/terraform-scripts ***********
  ...
  provisioner "file" {
      source      = "${var.pem_file}"
      destination = "/home/ec2-user/.ssh/${basename(var.pem_file)}"
  }

  provisioner "remote-exec" {
    inline = [
      "chmod 400 /home/ec2-user/.ssh/${basename(var.pem_file)}",
    ]
  }
  ...
  provisioner "remote-exec" {
    inline = [
      "mkdir -p /home/ec2-user/deploy_example",
  ...
  provisioner "file" {
      source = "../bash-scripts/"
      destination = "/home/ec2-user/deploy_example"
  }
  provisioner "remote-exec" {
    inline = [
      "chmod 774 /home/ec2-user/deploy_example/*.sh",
    ]
  }
...
#  provisioner "remote-exec" {
#    inline = [
#      "/home/ec2-user/deploy_example/deployETCD.sh /home/ec2-user/.ssh/${basename(var.pem_file)}",
#    ]
#  }
...
#  provisioner "local-exec" {
#    command = "ssh -f -i ${var.pem_file} -o 'StrictHostKeyChecking no' hadoop@${aws_instance.palisade_instance.public_dns} 'nohup /home/hadoop/deploy_example/deployConfigService.sh > /home/hadoop/example_logs/deployConfigService.log 2>&1 &'"
#  }
