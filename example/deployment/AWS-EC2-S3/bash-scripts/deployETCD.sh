...
sudo /opt/etcd-${ETCD_VER}/etcd \
  --name=etcd_s1 \
  --data-dir /etcd-data \
  --initial-advertise-peer-urls=http://${private_dns}:2380 \
  --listen-client-urls=http://${private_dns}:2379 \
  --advertise-client-urls=http://${private_dns}:2379 \
  --listen-peer-urls=http://${private_dns}:2380 \
  --initial-advertise-peer-urls http://${private_dns}:2380 \
  --initial-cluster=etcd_s1=http://${private_dns}:2380 \
