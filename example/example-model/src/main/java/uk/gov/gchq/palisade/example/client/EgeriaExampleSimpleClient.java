...
public class EgeriaExampleSimpleClient extends SimpleClient<Employee> {
    private static final Logger LOGGER = LoggerFactory.getLogger(RestExample.class);

    public EgeriaExampleSimpleClient(final PalisadeService palisadeService) {
    ...
    public static void main(final String[] args) throws InterruptedException {
        ...
        if (args.length == 3) {
            ...
            final Stream<Employee> results = new EgeriaExampleSimpleClient(palisade).read(filename, user.getUserId().getId(), purpose);
            LOGGER.info(user.getUserId().toString() + " got back: ");
            results.map(Object::toString).forEach(LOGGER::info);

        } else {
            System.out.printf("Usage: %s userId resource purpose\n\n", EgeriaExampleSimpleClient.class.getSimpleName());
